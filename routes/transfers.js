...
router.post('/api', async (req, res) => {
  try {
    ...
    if (!from_location_id || !to_location_id || !lines || lines.length === 0)
      return res.status(400).json({ error: 'Locations and at least one product required' });
    // Bug #7 partial fix: enforce locations differ at creation time
    if (String(from_location_id) === String(to_location_id))
      return res.status(400).json({ error: 'Source and destination must differ' });
    // Bug #17 fix for transfers: validate line quantities
    for (const line of lines) {
      if (!line.product_id || !line.quantity || parseFloat(line.quantity) <= 0) {
        return res.status(400).json({ error: 'Each line must have a product and a positive quantity' });
      }
    }
...
router.post('/api/:id/validate', async (req, res) => {
  ...
  try {
    ...
    const transfer = await client.query(
      `SELECT t.*, fl.name as from_loc, tl.name as to_loc, fw.name as from_wh, tw.name as to_wh
       FROM transfers t
       ...
       WHERE t.id = $1 AND t.status = 'draft'`, [req.params.id]);
    if (transfer.rows.length === 0) {
      await client.query('ROLLBACK');
      return res.status(400).json({ error: 'Transfer not found or already validated' });
    }
    const t = transfer.rows[0];
    const lines = await client.query(
      'SELECT tl.*, p.name as product_name, p.current_stock FROM transfer_lines tl JOIN products p ON tl.product_id = p.id WHERE tl.transfer_id = $1',
      [req.params.id]);

    for (const line of lines.rows) {
      // Bug #7 fix: check stock availability before allowing transfer
      if (parseFloat(line.current_stock) < parseFloat(line.quantity)) {
        await client.query('ROLLBACK');
        return res.status(400).json({
          error: `Insufficient stock for ${line.product_name}. Available: ${line.current_stock}, Requested: ${line.quantity}`
        });
      }
      // Transfer is a location move — global current_stock stays the same;
      // we only log a movement record to track the inter-location activity.
      await client.query(
        'INSERT INTO movements (product_id, from_location, to_location, quantity, operation_type, reference) VALUES ($1,$2,$3,$4,$5,$6)',
